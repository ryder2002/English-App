...
export function MainLayout({ children }: { children: React.ReactNode }) {
  ...
  const NavItem = ({ item, isMobile = false }: { item: NavItemType, isMobile?: boolean }) => {
    ...
    if (hasChildren) {
      return (
        <Collapsible open={isOpen} onOpenChange={() => toggleSection(item.label)} className="w-full">
      ...
      <aside className="hidden lg:flex flex-col w-[280px] h-screen fixed top-0 left-0 z-50 border-r border-border/40 bg-background/80 backdrop-blur-xl shrink-0">
        <div className="p-6 flex items-center gap-3">
          <div className="relative group">
            <div className="absolute inset-0 bg-gradient-to-br from-yellow-400 to-red-600 rounded-xl blur-lg opacity-50 group-hover:opacity-75 transition-opacity duration-500" />
            <div className="relative w-16 h-16 flex items-center justify-center group-hover:scale-105 transition-transform duration-300">
              <Image src="/Logo.png" alt="Logo" width={65} height={65} className="object-contain drop-shadow-lg" />
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-border/40 space-y-2">
          ...
          <Link
            ...
            className={cn(
              ...
              pathname === "/settings" && "bg-accent/50 text-foreground font-medium"
            )}
          >
        ...
        <div className="flex items-center gap-3">
          <Sheet open={isMobileOpen} onOpenChange={setIsMobileOpen}>
            <SheetTrigger asChild>
              <Button variant="ghost" size="icon" className="rounded-xl hover:bg-accent/10">
              ...
              <div className="flex flex-col h-full">
                <div className="p-6 flex items-center gap-3 border-b border-border/40">
                  <div className="relative">
                    <div className="absolute inset-0 bg-gradient-to-br from-yellow-400 to-red-600 rounded-xl blur opacity-40" />
                    <div className="relative bg-background rounded-xl p-2 border border-border/50">
                      <CNLogo className="w-8 h-8" />
                    </div>
                  </div>
                  <span className="font-bold text-xl tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-yellow-500 via-orange-500 to-red-600 animate-gradient-x">CN English</span>
                </div>
          ...
          <div className="flex items-center gap-2">
            <CNLogo className="w-6 h-6" />
            <span className="font-bold text-lg tracking-tight lg:hidden">CN English</span>
          </div>
        </div>
